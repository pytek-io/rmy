...
@receive_test_object_async
async def test_async_generator(proxy: TestObject):
    async for i, value in enumerate(proxy.count.wait(10)):
        assert i == value


@receive_test_object_async
async def test_sync_generator(proxy: TestObject):
    async for i, value in enumerate(proxy.count_sync.wait(10)):
        assert i == value


@receive_test_object_async
async def test_async_generator_exception(proxy: TestObject):
...
@receive_test_object_async
async def test_explicit_close(proxy: TestObject):
...
@receive_test_object_async
async def test_close_on_drop(proxy: TestObject):
...
@receive_test_object_async
async def test_overflow(proxy: TestObject):
...
@receive_test_object_async
async def test_remote_generator_pull_decorator(proxy: TestObject):
    with check_exception(OverflowError(ASYNC_GENERATOR_OVERFLOWED_MESSAGE)):
